...
class PerformRocketgateCompleteThreeDCommandHandler extends BaseCommandHandler
{
    /**
     * @var ChargeService
     */
     ...
     * @param InMemoryRepository                        $inMemoryRepository                        The Redis interface
    ...
    {
        try {
            if (!$command instanceof PerformRocketgateCompleteThreeDCommand) {
                throw new InvalidCommandException(PerformRocketgateCompleteThreeDCommand::class, $command);
            }

            Log::info('Begin processing complete threeD transaction');
            ...
            $cvv = $this->inMemoryRepository->retrieveCvv(
                (string) $previousTransaction->transactionId(),
                $previousTransaction->billerName()
            );

            // Perform complete threeD transaction
            $billerResponse = $this->chargeService->completeThreeDCreditCard(
                $previousTransaction,
                $command->pares(),
                $command->md(),
                $cvv
            ...
            // If we found a CVV stored for the transactionId and billerName ( key )
            if (!is_null($cvv)) {
                // Delete CVV from Redis as we don`t need it anymore
                $this->inMemoryRepository->deleteCvv(
                    (string) $previousTransaction->transactionId(),
                    $previousTransaction->billerName()
                );
            }
